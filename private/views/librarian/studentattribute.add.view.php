            ...
            <form id="myForm" method="post" enctype="multipart/form-data">
                ...
                <div class="bodyContainer03">
                    ...
                    <select id="Description" class="Description" name="Description" id="Description" value="<?= get_var('Description') ?>" required>
                        ...
                        <option <?= get_select('Description', 'degree') ?> value="degree">Degree</option>
                        <option <?= get_select('Description', 'batch') ?> value="batch">Batch</option>


                    </select>

                    <button class="addcategorybtn91" id="addcategorybtn181" name="paadd11">Display</button>

                    <button class="addcategorybtn18" id="addcategorybtn9" name="paadd112">Add</button>
                    ...
                    <table class="category_table" id="category_table">
                        ...
                        foreach ($rows as $row) : ?>
                            <tr>
                                ...
                                <td><button type='button' class='editbtn' id='editbtn'><i class='fa-solid fa-pen'></i>&nbsp;<a class="a1" href='<?= ROOT ?>/attributes/studentedit/<?= $row->Type ?>/<?= $row->ID ?>'>
                                            Edit</a></button>
                                    <button type='button' class='deletebtn' id='deletebtn' onclick="openStudentAttributeDeleteCategoryPopupView('<?= $row->ID ?>','<?= $row->Type ?>')"><i class='fa-solid fa-trash'></i>&nbsp;Delete</button>
                                </td>
